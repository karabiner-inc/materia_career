defmodule MateriaCareer.Messages do
  ...
  def get_my_project_offer!(_result, offer_id, project_id, status) do
    ...
    if length(offers) != 1 do
      raise BusinessError,
        message:
          "user organizations offer length was unexpected number. offer_id:#{offer_id} project_id:#{project_id} length:#{
            length(offers)
          }"
    end
  ...
  def get_my_offer!(_result, offer_id, from_user_id, status) do
    ...
    if length(offers) != 1 do
      raise BusinessError,
        message:
          "user offer length was unexpected number. offer_id:#{offer_id} from_user_id:#{from_user_id} length:#{
            length(offers)
          }"
    end
  ...
  def get_my_offer_and_to_me!(_result, offer_id, user_id, status) do
    ...
    if length(offers) != 1 do
      raise BusinessError,
        message:
          "user offer length was unexpected number. offer_id:#{offer_id} user_id:#{user_id} length:#{length(offers)}"
    end
  ...
  def get_offer_to_me!(_result, offer_id, to_user_id, status) do
    ...
    if length(offers) != 1 do
      raise BusinessError,
        message:
          "user offer length was unexpected number. offer_id:#{offer_id} to_user_id:#{to_user_id} length:#{
            length(offers)
          }"
    end
  ...
  def switch_send_mail(attr) do
    if Map.get(attr, "to_user_id") == nil do
      ...
      project =
        Projects.get_project!(attr["project_id"])
  ...
  defp send_email(template_type, from_email, email, replace_list, from_name \\ nil) do
    if from_email == nil do
      raise BusinessError, message: "config :materia, MateriaCareer.Offer, system_from_email not found."
    end
